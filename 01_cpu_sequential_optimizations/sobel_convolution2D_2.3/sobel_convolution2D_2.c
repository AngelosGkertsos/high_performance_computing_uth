...
int convolution2D(int posy, int posx, const unsigned char *input, char operator[][3]) {
    int j, res, temp_j_operator, temp_horizontal_pos;
    
    res = 0;
    for (j = -1; j <= 1; j++) {

        temp_j_operator = j+1;
        temp_horizontal_pos = posx + j;

        res += input[(posy - 1)*SIZE + temp_horizontal_pos] * operator[0][temp_j_operator];
        res += input[(posy)*SIZE + temp_horizontal_pos] * operator[1][temp_j_operator];
        res += input[(posy + 1)*SIZE + temp_horizontal_pos] * operator[2][temp_j_operator];
