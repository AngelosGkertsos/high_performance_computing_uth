...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden, int silent_mode)
{
    ...
    unsigned char *row_1;
    unsigned char *row_2;
    unsigned int cur_row, cur_pixel;
    ...
    for (j=1; j<SIZE-1; j+=1) {
        for (i=1; i< (SIZE-1) - 15; i+=16 ) {
            ...
            if(p > 65025){p = 65025;} output[cur_pixel] = sqrt_lookup_table[p];
            diff = (double)output[cur_pixel] - (double)golden[cur_pixel];
