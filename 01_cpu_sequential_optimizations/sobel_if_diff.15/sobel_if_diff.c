...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden, int silent_mode)
{
    ...
    int i, j;
    unsigned int p;
    int gradient_x, gradient_y, col_right, temp_i;
    ...
    for (j=1; j<SIZE-1; j+=1) {
        for (i=1; i< (SIZE-1) - 15; i+=16 ) {
            ...
            unsigned char *row_0 = &input[(j-1) * SIZE];
            unsigned char *row_1 = &input[j * SIZE];
            unsigned char *row_2 = &input[(j+1) * SIZE];
            ...
            // Left column
            gradient_x = row_0[temp_i] * (-1); gradient_y = row_0[temp_i];
            gradient_x += row_1[temp_i] * (-2); 
            gradient_x += row_2[temp_i] * (-1); gradient_y += row_2[temp_i] * (-1);
            
            // Middle collumn
            temp_i++;             
            gradient_y += row_0[temp_i] << 1;
            gradient_y += row_2[temp_i] * (-2);

            // Right column
            col_right = temp_i + 1;
            gradient_x += row_0[col_right]; gradient_y += row_0[col_right];
            gradient_x += row_1[col_right] << 1;
            gradient_x += row_2[col_right]; gradient_y += row_2[col_right] * (-1);

            p = (gradient_x * gradient_x) + (gradient_y * gradient_y);
            if(p > 65025){p = 65025;} output[j * SIZE + temp_i] = sqrt_lookup_table[p];
            ...
            temp_i++; //now temp_i is indexing collumn
            gradient_y += row_0[temp_i] << 1;
            gradient_y += row_2[temp_i] * (-2);            
            ...
            //gradient_x= 0;
            //gradient_y = 0;
            //gradient_x += row_0[col_left] * (-1); gradient_y += row_0[col_left];
            //gradient_x += row_1[col_left] * (-2);
            //gradient_x += row_2[col_left] * (-1); gradient_y += row_2[col_left] * (-1); 
            //gradient_y += row_0[temp_i] << 1;
            //gradient_y += row_2[temp_i] * (-2);           
            //gradient_x += row_0[col_right]; gradient_y += row_0[col_right];
            //gradient_x += row_1[col_right] << 1;
        ...
        for(; i < SIZE - 1; i++){
            ...
            gradient_x = row_0[i-1] * (-1); gradient_y = row_0[i-1];
            gradient_x += row_1[i-1] * (-2);
            gradient_x += row_2[i-1] * (-1); gradient_y += row_2[i-1] * (-1); 
            gradient_y += row_0[i] << 1;
            gradient_y += row_2[i] * (-2);           
            gradient_x += row_0[i+1]; gradient_y += row_0[i+1];
            gradient_x += row_1[i+1] << 1;
            gradient_x += row_2[i+1]; gradient_y += row_2[i+1] * (-1);   
            p = (gradient_x * gradient_x) + (gradient_y * gradient_y);
            if(p > 65025){p = 65025;} output[j * SIZE + i] = sqrt_lookup_table[p];
