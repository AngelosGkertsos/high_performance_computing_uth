...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden, int silent_mode)
{
    ...
    for (j=1; j<SIZE-1; j+=1) {
        for (i=1; i< (SIZE-1) - 15; i+=16 ) {
            ...
            // Left column
            gradient_x += input[row_0 + col_left] * (-1); gradient_y += input[row_0 + col_left];
            gradient_x += input[row_1 + col_left] * (-2); 
            gradient_x += input[row_2 + col_left] * (-1); gradient_y += input[row_2 + col_left] * (-1);
            
            // Middle collumn             
            gradient_y += input[row_0 + temp_i] * 2;                          
            gradient_y += input[row_2 + temp_i] * (-2);
            
            // Right column
            gradient_x += input[row_0 + col_right]; gradient_y += input[row_0 + col_right];
            gradient_x += input[row_1 + col_right] * 2; 
            gradient_x += input[row_2 + col_right]; gradient_y += input[row_2 + col_right] * (-1);
        ...
        for(; i < SIZE - 1; i++){
            ...
            gradient_x += input[row_1 + col_left] * (-2); 
            gradient_x += input[row_2 + col_left] * (-1); gradient_y += input[row_2 + col_left] * (-1); 
            gradient_y += input[row_0 + i] * 2;
            gradient_y += input[row_2 + i] * (-2);           
            gradient_x += input[row_0 + col_right]; gradient_y += input[row_0 + col_right];
            gradient_x += input[row_1 + col_right] * 2; 
            gradient_x += input[row_2 + col_right]; gradient_y += input[row_2 + col_right] * (-1);
            p = (gradient_x * gradient_x) + (gradient_y * gradient_y);
