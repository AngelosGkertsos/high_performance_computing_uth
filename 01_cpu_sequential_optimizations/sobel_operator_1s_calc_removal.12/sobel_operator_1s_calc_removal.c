...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden, int silent_mode)
{
    ...
    for (j=1; j<SIZE-1; j+=1) {
        for (i=1; i< (SIZE-1) - 15; i+=16 ) {
            ...
            // Left column
            gradient_x += input[row_0 + col_left] * horiz_operator[0][0]; gradient_y += input[row_0 + col_left];
            gradient_x += input[row_1 + col_left] * horiz_operator[1][0]; 
            gradient_x += input[row_2 + col_left] * horiz_operator[2][0]; gradient_y += input[row_2 + col_left] * vert_operator[2][0];
            
            // Middle collumn             
            gradient_y += input[row_0 + temp_i] * vert_operator[0][1];                          
            gradient_y += input[row_2 + temp_i] * vert_operator[2][1];
            
            // Right column
            gradient_x += input[row_0 + col_right]; gradient_y += input[row_0 + col_right];
            gradient_x += input[row_1 + col_right] * horiz_operator[1][2]; 
            gradient_x += input[row_2 + col_right]; gradient_y += input[row_2 + col_right] * vert_operator[2][2];
        ...
        for(; i < SIZE - 1; i++){
            ...
            gradient_x += input[row_1 + col_left] * horiz_operator[1][0]; 
            gradient_x += input[row_2 + col_left] * horiz_operator[2][0]; gradient_y += input[row_2 + col_left] * vert_operator[2][0]; 
            gradient_y += input[row_0 + i] * vert_operator[0][1];
            gradient_y += input[row_2 + i] * vert_operator[2][1];           
            gradient_x += input[row_0 + col_right]; gradient_y += input[row_0 + col_right];
            gradient_x += input[row_1 + col_right] * horiz_operator[1][2]; 
            gradient_x += input[row_2 + col_right]; gradient_y += input[row_2 + col_right] * vert_operator[2][2];
            p = (gradient_x * gradient_x) + (gradient_y * gradient_y);
...
//    for(j=1; j <SIZE-1; j++){
//        unsigned char *ptr_row_0 = &input[(j-1) * SIZE];
//        unsigned char *ptr_row_1 = &input[j * SIZE];
//        unsigned char *ptr_row_2 = &input[(j+1) * SIZE];
//
//        
//
//    }
